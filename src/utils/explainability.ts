import { Incident, FactorContribution } from '../types/incident';
import { calculateFactorContributions, FACTOR_LABELS } from './scoringEngine';
...
export interface ComparisonDelta {
  ...
  pointDiff: number; // Positive means A gained more points than B from this factor
  explanation: string;
}
...
/**
 * Generates an explainable reasoning dossier for an incident's ranking
 */
export function generateRankingExplanation(
  ...
  if (incident.correlatedIncidentIds.length > 0) {
    ...
    const sharedIps = Array.from(
      new Set(correlatedIncidents.map((i) => i.sourceIp).filter((ip) => ip === incident.sourceIp))
    );
    const sharedUsers = Array.from(
      new Set(correlatedIncidents.map((i) => i.user).filter((u) => u === incident.user))
    );
    ...
    }, elevating its correlation score to ${incident.factors.correlationScore}/10 (+${(
      incident.factors.correlationScore * 1.0
    ).toFixed(1)} pts).`;
  } else {
...
/**
 * Compares two incidents and provides an explainable factor-by-factor delta
 * (e.g., "Why Incident #1 outranks Incident #2")
 */
export function compareIncidentsExplainable(
  ...
  const contributionsHigher = calculateFactorContributions(higher.factors);
  const contributionsLower = calculateFactorContributions(lower.factors);
  ...
  // Find biggest driver of the difference
